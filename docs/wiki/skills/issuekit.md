...
It also sweeps **open** layer PRs, which catches the drift before the merge rather than after. That sweep needs a different repair from the plain one, so it sorts each PR into a case first. A layer whose keyword is inert is **expected and gets left alone** — rewriting the body repairs nothing, and retargeting to trunk would destroy the stack. A layer with no keyword at all is a real gap, and the body edit is worth making now so the link registers the moment the retarget lands. A PR GitHub has already retargeted but never re-parsed gets its body written back unchanged, which forces the re-parse.
