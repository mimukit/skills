...
**The rest of the map serves the skills downstream of it.** Update-branch puts the button mergekit's sync step reaches for. Auto-merge is the one row no skill here consumes: [`mergekit`](./mergekit.md) merges on your explicit word and [`afkkit`](./afkkit.md) stops at an open PR, so `setup` proposes it as a convenience for the human merging by hand, not as a dependency. Both secret-scanning rows are free on a public repo and need Advanced Security on a private one, so on a private repo `setup` says the plan may reject them and offers them anyway — a refused flag costs one error message, and a leaked credential costs a rotation. Neither `gh repo view` field set covers update-branch or secret scanning, so `setup` reads those two from the REST API.
