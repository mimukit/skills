...
Full runs add four cross-file checks that a per-skill run (`make lint name=<skill>`) deliberately skips:
...
- **The cheatsheet.** [`docs/wiki/cheatsheet.md`](./cheatsheet.md) is generated by `scripts/cheatsheet.sh` from the per-skill pages, so lint regenerates it into a temp file and diffs. Nothing on the page is authored, which is what makes the check unambiguous: a difference always means a skill page moved and nobody ran `make cheatsheet`. Deriving the page rather than writing it is the point — a hand-kept recall list of 34 skills and their modes goes stale in a week, and no reader can tell.
