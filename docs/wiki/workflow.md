...
Several kits stay off the map entirely, and for three different reasons. `repokit` runs *before* the loop, once per repo, provisioning the label vocabulary everything downstream reads. `afkkit` runs the whole START → SHIP span unattended and calls `issuekit start` itself at the front, so it replaces a stretch of the map rather than adding to it. Everything else is invoked on demand and enters from wherever you already are: `handoffkit`, `humankit`, `skillkit`, `promptkit`, `refactorkit`, `debugkit`, `designkit`, `orcakit`, and `tutorkit`. Two of those are worth singling out. `promptkit` runs before you invoke anything at all, so a box for it would imply a step that doesn't exist. `debugkit` enters from a *symptom* rather than from a phase, which is why nothing on the map points at it — you arrive there because something broke, not because you finished the row above.
...
- **`designkit`** — the design-system counterpart to `wikikit`, keeping a second in-repo contract true to the code. It writes `DESIGN.md` at the repo root — the open Google Labs format that pairs machine-readable tokens in YAML front matter with the rationale in prose — because that's where DESIGN.md-aware tools look for it, `docs/` being tidier and wrong. Same three modes as `wikikit` and the same posture: `init` derives the file, `update` applies what the code now says, `audit` is read-only. **The grounding rule is the whole skill: every token must be a value that already appears in your codebase.** Clustering forty near-identical greys into a scale is derivation; picking a nicer neighbouring hex because it rounds better is invention, and that's the line it never crosses. It shells out to the official CLI for linting and contrast checks rather than reimplementing them, and it **generates no UI** — that's `uikit`, which reads `DESIGN.md` and never writes it.
...
- **`tutorkit`** — the only kit that targets the person rather than the repo. Every other skill here assumes you already know the thing and makes you faster at it; this one is for ending up knowing it. State lives in a separate learning repo at `~/learning` (`$TUTORKIT_HOME` overrides), one folder per topic, so it writes nothing into the project you're working in. Five modes: `status` reads two router files and prints where every track stands, `explain` answers and writes nothing, `lesson` is the only mode that opens a track, `drill` runs spaced retrieval interleaved across topics, `exam` places you at the start and runs a transfer test at the end. Two devices carry it — **predict before you explain**, because a wrong prediction names the broken mental model that's actually blocking the correct one, and an **explain-back gate** that keeps a lesson open until you restate the concept yourself. When cwd is a real project it asks **once** for consent to read it, then builds examples out of your own code. It sits outside the loop entirely and doesn't route into it; the only sideways moves are to `researchkit` when the question turns out to be a tool decision, and to `prototypekit` when the honest answer is to go build the thing.
