...
- **Every API endpoint gets a runnable `curl`** — whenever a case or check exercises an HTTP endpoint, include the exact `curl` invocation in its own ```sh block rather than describing the request in prose. Spell out the method, full URL (with the local base URL or a `$BASE_URL` the Preconditions define), every required header, and a concrete JSON body with real sample values — copy-paste-ready, no `<placeholders>` the tester has to guess at. Use `-i` (or `-s -w '\n%{http_code}\n'`) when the expected result includes a status code, and note in **Preconditions** how to obtain any auth token the calls need. This is the single biggest speedup in a QA pass: the tester runs the request instead of reconstructing it.
...
- **Anything that destroys or rebuilds state** — `*:destroy`, `*:reset`, a teardown-and-rescaffold, a database drop, a `clean` that wipes a build. Describe it for the human in **Preconditions** or the case itself; never execute it. You are writing a plan *about* an environment, not administering one, and a QA agent that resets state can wipe the very build the human was about to test.
