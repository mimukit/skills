...
- **What's in it.**
  ...
  | `verified.md` | [Verify](#4-verify), refreshed by [the fix loop](#6-fix-loop) | [Review](#5-review), [QA plan](#7-qa-plan) |
...
|------|-------|------|-----------|-----|
| Start + spec gate | `opus` | 1× | ~27–60 | One dispatch: issuekit `start`, then the gate in the worktree it acquired (see [Start the issue](#1-start-the-issue)). The gate decides the whole run, since a missed decision-gap poisons every step after it, and every later step inherits its orientation and its check list. Runs once, so buying capability here is nearly free. |
| Implement | `opus` | 1× per phase group | ~20–175 per group, scales with the change | The bulk of the work; implementkit's own test + build gate is the safety net underneath it. It commits its own work before returning. |
| Verify | `opus` | 1× | ~25–65 | Runs the code rather than reading it, the one lens review does not have. Small context (orientation plus the check list), so the strong tier is cheap here. |
| Review, round 1 | `fable` | 1× | ~11–29 | The quality gate, over the full branch diff, deliberately on a **different model family** from the one that wrote the code; see below. |
| Review, rounds 2–3 | `opus` | ≤2× | ~10–20 | Delta-scoped to the fix commits and the surviving findings (see [Fix loop](#6-fix-loop)). Independence is bought once, on round 1; see below for why the delta rounds left `fable`. |
| Fix | `opus` | ≤2× | ~15–120, scales with the findings | Applying review findings against a concrete list. It re-runs the checks its changes touch, refreshes `verified.md`, and commits its own work before returning. |
...
Start shares one dispatch with the [spec gate](#2-spec-gate): dispatch a subagent, on the gate's tier, that invokes **issuekit** `start <n>` and, when start succeeds, continues straight into the gate inside the worktree it just acquired. afkkit adds nothing to `start` and re-implements none of it: issuekit refuses any issue not labeled `ready`, asks **gitkit** for the worktree (branch `issue-<n>-<slug>`, cut from the resolved base ref, adopting an existing one rather than recreating it), and flips the label `ready → in-progress`.

Folding the two is [the dispatch floor](#how-the-conductor-runs-each-step) read from the other side: start alone is 3–7 tool uses under a full dispatch floor (each measured run paid ~35–44k tokens for that floor), and the gate's first need is the worktree path the same agent is already holding.

**The refusal path costs the gate nothing.** This step is the only place the `ready` guard becomes legible to afkkit, and its result is not a boolean: issuekit can refuse four distinguishable ways, each routing somewhere different, and one of them isn't a refusal at all. A relay that flattens that distinction breaks the escalation policy silently. On a refusal the subagent returns a structured refusal naming **which** of the cases below it hit, verbatim, and never begins the gate. On success it returns `{worktree, branch, label}` together with the gate's outputs.
...
**Say the run is unattended when you invoke it.** issuekit previews every mutation and waits for an OK, except its label writes: every label write runs unprompted for every caller, so afkkit inherits `start`'s `ready → in-progress` flip without asking. Saying the run is unattended still matters, because it tells every later step there's nobody to answer a prompt. prkit exempts its label writes the same way ([the `in-review` advance](#8-open-the-pr)); nothing outside the label namespaces is exempt, and afkkit never asks for a broader carve-out.

Then verify what came back rather than taking it on faith. The conductor runs this itself, since reading the workspace is squarely inside its boundary, and it runs the three checks as **one** shell call, never three: every conductor tool use re-bills the conductor's whole context, so unbatched bookkeeping reads are pure overhead.
...
The gate runs in [the start dispatch](#1-start-the-issue): the same agent that acquired the worktree reads the issue body and the relevant code, and classifies any gaps between what the issue specifies and what building it requires. Because it is the only step that explores the repo before any code exists, it is also the step that writes down everything the run will need later. It has **five outputs**: the classification below, the issue's phase grouping, plus `orientation.md`, `assumptions.md`, and `checks.md` in [the run directory](#the-run-directory). Writing all three files costs the gate almost nothing; it is already holding everything that goes in them.

**The phase list is a return value, not a file, and the gate groups it.** An issue written by issuekit carries its phases as `## Phase N` headings. The gate reads the whole body anyway, so it returns the headings in order, **grouped into dispatch groups**: a phase it judges heavy stands alone, and consecutive light phases fold into one group. The test is whether a phase needs a fresh context of its own, not whether the issue gave it a heading, and the gate is the only step positioned to make that call cheaply, since it has just read the code every phase touches. In a measured run, four phases of ~22 tool uses each ran as four dispatches and paid four dispatch floors for work two would have carried. [Implement](#3-implement) dispatches one subagent per group. A body with no phase headings returns a single unnamed group, which is the same thing as one dispatch.
...
**A multi-phase issue gets one implement dispatch per phase group**, in the order the phases are written, each invoking implementkit narrowed to its group's phases and each committing before it returns. A single-group issue gets exactly one dispatch, which is this step's original shape and its original cost. The grouping comes from the [spec gate](#2-spec-gate), which read the whole issue body already and folded consecutive light phases together; the conductor holds it and never re-reads the issue to rebuild it.
...
Route a failure exactly as a single-group run does, per group. Then name the phases in what the conductor holds:

- **A group escalates** → the run stops there. The escalation comment names the phases already committed, the phase that stopped, and why, so a re-run resumes at the right place instead of rebuilding what landed.
- **A group reports green** → its commits are on the branch. Dispatch the next group.
...
**The conductor does not tick the issue's phase checkboxes as phases land.** It would read as useful progress on a long run, and it is a new unprompted tracker mutation that no mode owns. The exemptions in this pipeline cover label writes only ([issuekit `start`'s flip](#1-start-the-issue) and [prkit's `in-review` advance](#8-open-the-pr)), they belong to the owning skill rather than to afkkit, and a checkbox edit is not a label write. The commits on the branch are the record while the run is live, and the PR is the record after it.
...
Four hard rules, all of them about not re-doing paid work: **never edit code or copy files into the workspace**, because a failure is evidence for [Review](#5-review), not something this step works around; **never rebuild, rescaffold, or install**, with the single exception of a change whose subject *is* the build or scaffold path; **never re-derive** a fact `orientation.md` already holds; and when a check needs a dev server, **start it once, run every check that needs it, stop it, and record the exact start and stop commands in `verified.md`** so no later step rediscovers them. The rebuild rule is the one that breaks in practice: in a measured run this step destroyed and rescaffolded a sandbox app, re-ran the package install, and reset the sandbox twice, which made it the longest dispatch after the implement phases (111 tool uses) for checks the built tree already supported.
...
Dispatch a subagent (worktree) to invoke **qakit**, which writes a manual QA plan grounded in the diff to `docs/qa/qa-<slug>-YYYY-MM-DD.md` and fills its **Automated verification** section from the recorded check outcomes.

**By the time this runs, the checking is done.** Hand it `checks.md`, `verified.md`, `orientation.md`, the fact that the gate is green, and where the build output lands. `verified.md` is current, because [Verify](#4-verify) ran every check and [each fix round](#6-fix-loop) refreshed the entries its changes touched. So this step **transcribes the recorded outcomes and writes the manual cases; it re-runs nothing**, starts no server, and issues no requests. **It does not design a check set from scratch**, and it does not re-derive the acceptance criteria: the gate already wrote them down and Verify already exercised them. The human-only entries the gate flagged in `checks.md` are the manual test cases, which is qakit's own split arriving pre-made. In a measured run this step restarted the dev servers and repeated the same request flows Verify had finished minutes earlier, 53 tool uses to reproduce information the run was already holding; the transcription shape costs about 15.
...
Print one table per issue, after the outcome and before the next move. One row per dispatched step, so [a per-group implement dispatch](#one-dispatch-per-phase) gets its own row and names its phases (`Implement · phase 2`, or `Implement · phases 2–3` for a folded group):

| Step | Model | Time | Tool uses | Tokens |
|------|-------|------|-----------|--------|

The conductor fills this from what each dispatch reports back, and it measures nothing itself and runs no extra call to find out. **Print only the columns the harness actually gives you.** If a host reports no token count, drop that column and say the host doesn't report it. Never estimate a number into it; a made-up figure here is worse than a missing one, because the whole point of the table is to correct [the routing table](#model-routing) from real data instead of one remembered run. **When the harness reports cache reads and cache writes, print those two columns as well.** The headline per-dispatch token figure understates real billing by an order of magnitude, because cache reads dominate an agent's cost: in a measured run the dispatches reported ~1.1M tokens while ~56M cache-read tokens billed underneath them, and a routing decision tuned against the headline number optimizes the wrong thing.
...
**`stacked` issues are passed over too, and the preview must say so.** An issue whose prerequisite has an open PR is workable by hand, and issuekit `start` will accept it, but a batch cannot safely chain them: every layer depends on the layer below staying green, so one bad bottom layer silently poisons everything above it, and restructuring a stack needs an interactive tool no unattended run can drive. So the batch skips them, and because the up-front preview promises to name what it left out, **list the skipped `stacked` issues with their priorities and say chain-draining is not supported yet**. Silence here is the failure worth avoiding: a user who sees only the `ready` queue assumes the rest is unworkable, when in fact it is workable and only *this* runner declines it. Name them explicitly (`afkkit 44`) to run one, or work the chain by hand.
...
- **Two conductors are slower than one, and this covers hand-started ones too.** Parallel afkkit sessions in separate terminals share one usage budget. A measured pair started in the same minute stalled each other into a session limit for three and four hours mid-run, and one paid for a PR dispatch that died at the limit and had to be dispatched again. Queue the issues into one conductor instead.
...
- **No stack chain-draining.** A batch will not walk a chain of `stacked` issues, building each layer on the one below. The pipeline runs a named `stacked` issue like any other, because issuekit's guard already verifies the prerequisite's PR is live, but `all` skips them and says it did. Chaining is deferred rather than rejected: a broken bottom layer invalidates every layer above it, and repairing a stack needs an interactive tool nothing unattended can drive.
