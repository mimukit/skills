...
name: paseokit
description: >-
  Push the git worktrees your workflow actually creates into Paseo's workspace registry, and reap the rows whose directories are gone — Paseo discovers nothing on its own and prunes nothing on its own. Use when the user says "paseokit", "sync my worktrees to paseo", "my paseo sidebar is missing worktrees", "paseo doesn't show my worktree", "clean up my paseo workspaces", "my paseo sidebar is full of dead entries", or "where should paseo put new worktrees".
license: MIT
...
[Paseo](https://paseo.sh) shows one **workspace** per checkout — a main repo, or a git worktree under it — and hangs agents off each one. Worktrees themselves come from plain `git worktree`: gitkit's convention, issuekit's `start`, or your own hands.
...
paseokit works one layer up, on the only thing Paseo alone knows: **the registry row** — that a workspace exists for this path, under this project, with this title.

Two consequences worth stating plainly:

- **`paseo workspace archive` is registry-only.** It removes the row and leaves the directory, the branch, and git's own worktree registration completely intact. That is what lets [`sync`](#mode-sync) reap without a destructive-confirm gate — nothing it does can lose work.
...
**Not this skill:** creating or removing a worktree (gitkit), closing an issue and tearing down after a merge (issuekit `close`), or driving agents — `paseo run`, `attach`, `send`, `logs`, and the schedule surface all belong to the `paseo` CLI directly. paseokit is registry *hygiene*, not agent *operation*.
...
- **A rejected `paseo` flag or subcommand** → the CLI moves fast. Check `paseo <command> --help` before concluding an operation is unsupported. The goal — the row registered, the row archived, the title set — is the contract; the exact flag spelling is not.
...
**The CLI listing is a thin projection.** `paseo workspace ls --json` returns only `workspaceId`, `project` (the display name, not the id), `name`, `isolation`, and `cwd`. It lists **active workspaces only**. That is enough to answer one question — is any live row pointing at this path — and nothing else.
...
A workspace carries a **title and a pin, and nothing else** — no issue link, no status field. So `link`-style enrichment has no equivalent here; the title is the entire surface.
...
**Where it landed** — one table, with the counts per verdict.
...
Run [`list`](#mode-list)'s join first — `sync` acts on exactly those verdicts.
...
**What changed** — registrations, archives, collapses, and retitles, each with a count. Name every skip with its reason. Put `busy` skips first, because those are live work.
...
- **any `stray project`** → name it. Say that the fix is manual: edit `~/.paseo/projects/projects.json`, then run `paseo restart`. Do not perform it — a daemon restart kills every running agent, including the one reading this.
...
**Check the worktree root.** Compare `worktrees.root` in `~/.paseo/config.json` against `$WORKTREE_ROOT` — gitkit's convention, defaulting to `~/worktrees` unless the environment says otherwise. Two roots in play means every sweep classifies by path forever.
...
**What changed** — which config keys you compared, and which the user chose to change.

**Where it landed** — `~/.paseo/config.json`. Repeat that existing worktrees stayed where they were.

**Next** — run [`list`](#mode-list) to see the whole set in one table. Then stop. This is a one-time setting per machine, not a routine.

## Notes

- **paseokit is machine-local and always optional.** No Paseo on the box means no-op, and nothing else in the workflow may depend on it. gitkit, issuekit, and the rest never call it — they would break on every machine without the tool. It is a pump you run, not a link in a chain.
...
- **No shell available?** Then you cannot reach the `paseo` CLI, `git`, or `gh`. Reason from what the user gives you and **print the exact commands** as a codeblock for them to run — never report a workspace registered or archived that you could not perform.
